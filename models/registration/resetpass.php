...
if(isset($_GET['id']) && isset($_GET['code']))
{
	...
	function GenSalt ($length=10) {
     ...
     return $Salt;
			...
			$password = hash("sha256",(hash("sha256", $cpass).$salt));
			
				$stmt = $user->runQuery("UPDATE users SET userPass=:upass, salt=:usalt WHERE userID=:uid");
				$stmt->execute(array(":upass"=>$password, ":usalt"=>$salt, ":uid"=>$rows['userID']));
				
				$msg = "<div class='alert alert-success'>
						<button class='close' data-dismiss='alert'>&times;</button>
						Пароль изменен
						</div>";
				header("refresh:5;http://zmagar.ru/");
			}
		}	
	}
	else
