...
class Editing_db  //класс для работой с БД
{
	...
	public function create($fname,$lname,$email,$contact)
	{
		try
		{
			$stmt = $this->conn->prepare("INSERT INTO users(userName,PublicUserName,userEmail,datereg,date) VALUES(:fname, :lname, :email, :contact)");
	...
	public function update($id,$fname,$lname,$email,$contact)
	{
		try
		{
			$stmt=$this->conn->prepare("UPDATE users SET first_name=:fname, 
		                                               last_name=:lname, 
													   email_id=:email, 
													   contact_no=:contact
													WHERE id=:id ");
	...
	public function dataview($query)
	{
		...
		if($stmt->rowCount()>0)
		{
			while($row=$stmt->fetch(PDO::FETCH_ASSOC))
			{
                ...
                <td><?php print($row['PublicUserName']); ?></td>
                <td><?php print($row['datereg']); ?></td>
...
}  //конец класса для работой с БД

} //конец условия с проверкой 
